...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Luckycoin: increase each by adding 2 to luckycoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Luckycoind: Reduce the exempted free transactions to 500 bytes (from Luckycoin's 3000 bytes)
...
static bool fGenerateLuckycoins = false;
...
void GenerateLuckycoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateLuckycoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateLuckycoins = false;
    ...
    if (fGenerate)
    {
        ...
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadLuckycoinMiner, pwallet))
                printf("Error: CreateThread(ThreadLuckycoinMiner) failed\n");
            Sleep(10);
        }
    }
}
