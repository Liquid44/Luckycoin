...
// Copyright (c) 2011-2012 The Luckycoin Developers
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateLuckycoins(fGenerate, pwalletMain);
    return Value::null;
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    return CLuckycoinAddress(keyID).ToString();
}


CLuckycoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CLuckycoinAddress(account.vchPubKey.GetID());
}
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    CLuckycoinAddress address(params[0].get_str());
    if (!address.IsValid())
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CLuckycoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CLuckycoinAddress& address = item.first;
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    CLuckycoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    // Luckycoin address
    CLuckycoinAddress address = CLuckycoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Luckycoin address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    string strAccount = AccountFromValue(params[0]);
    CLuckycoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Luckycoin address");
    int64 nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CLuckycoinAddress> setAddress;
    vector<pair<CScript, int64> > vecSend;

    int64 totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CLuckycoinAddress address(s.name_);
        if (!address.IsValid())
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    return CLuckycoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CLuckycoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CLuckycoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CLuckycoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& s, listSent)
        {
            ...
            entry.push_back(Pair("address", CLuckycoinAddress(s.first).ToString()));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.push_back(Pair("address", CLuckycoinAddress(r.first).ToString()));
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("luckycoin-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("luckycoin-lock-wa");
...
public:
    ...
    Object operator()(const CScriptID &scriptID) const {
        ...
        BOOST_FOREACH(const CTxDestination& addr, addresses)
            a.push_back(CLuckycoinAddress(addr).ToString());
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "See https://en.luckycoin.it/wiki/BIP_0022 for full specification.");
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("luckycoin-rpclist");
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("luckycoin-rpchand");
